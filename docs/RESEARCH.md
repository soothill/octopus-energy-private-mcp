...
The query does not expose REST-style pagination metadata. To keep the MCP’s configured response limit meaningful, oversized EV charge arrays are rejected before caching or summarisation rather than silently truncated; callers can shorten the period or choose a coarser frequency.
